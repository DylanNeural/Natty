...
export default function ScannerResult() {
  ...
  const nutritionDetails = [
    { label: 'Sucres', value: r.nutritionDetails?.sucres, unit: 'g' },
    { label: 'Satures', value: r.nutritionDetails?.satures, unit: 'g' },
    { label: 'Fibres', value: r.nutritionDetails?.fibres, unit: 'g' },
    { label: 'Sel', value: r.nutritionDetails?.sel, unit: 'g' },
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: insets.bottom + 100 }} showsVerticalScrollIndicator={false}>
        ...
        >
          ...
          {nutritionDetails.length > 0 ? (
            <View style={{ marginTop: 12, padding: 12, borderRadius: 14, backgroundColor: 'rgba(0,65,47,0.06)' }}>
              <Text style={{ fontSize: 10, fontWeight: '700', color: C.darkSoft, letterSpacing: 1 }}>
                DETAILS NUTRITIONNELS
              </Text>
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>
                {nutritionDetails.map((item) => (
                  <View key={item.label} style={{ paddingVertical: 6, paddingHorizontal: 10, borderRadius: 12, backgroundColor: C.white }}>
                    <Text style={{ fontSize: 10, color: C.darkSoft, fontWeight: '700' }}>{item.label}</Text>
                    <Text style={{ fontSize: 13, color: C.dark, fontWeight: '800', marginTop: 2 }}>
