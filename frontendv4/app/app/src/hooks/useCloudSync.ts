...
export function useCloudSync() {
  ...
  useEffect(() => {
    ...
    pullProfile().catch(() => {});
    pullJournal().catch(() => {});
    pullReservations().catch(() => {});
  }, [isAuthenticated]);

  // ─── Push profil debounced ───────────────────────────────────
  const profileTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const schedulePushProfile = () => {
  ...
  useEffect(() => {
    ...
    const unsubUser = useUserStore.subscribe((state, prev) => {
      ...
      if (changed) schedulePushProfile();
    });

    // Push aussi quand les favoris changent (ils vivent dans profile.favorites)
    const unsubFav = useFavoritesStore.subscribe(() => schedulePushProfile());
    ...
    const unsub = useReservationsStore.subscribe((state, prev) => {
      ...
      state.reservations.forEach((r) => {
        ...
        if (
          ...
          pushReservation(r).catch(() => {});
        }
      });
      // Suppression
      prev.reservations.forEach((r) => {
        if (!currById.has(r.id)) deleteReservationCloud(r.id).catch(() => {});
      });
    });
    return unsub;
  }, [isAuthenticated]);
